...
export class VentaItem {
  ...
  static crear(props: {
    ...
    if (!props.id || props.id.trim().length === 0) {
      throw new DomainError('El ID del item no puede estar vacio', 'VENTA_ITEM_ID_VACIO');
    }
    if (!props.varianteId || props.varianteId.trim().length === 0) {
      throw new DomainError('El VarianteId no puede estar vacio', 'VENTA_ITEM_VARIANTE_VACIO');
    }
    if (!Number.isInteger(props.cantidad) || props.cantidad <= 0) {
      throw new DomainError('La cantidad debe ser un entero mayor que 0', 'VENTA_ITEM_CANTIDAD_INVALIDA');
    }
    const precioSnapshot = Precio.crear(props.precioSnapshot);
    return new VentaItem(props.id, props.varianteId, props.cantidad, precioSnapshot);
  }
  ...
  get precioSnapshot(): Precio { return this._precioSnapshot; }
}
