...
export class Producto {
  ...
  static crear(props: {
    ...
    if (!props.id || props.id.trim().length === 0) {
      throw new DomainError(
        'El ID del producto no puede estar vacio',
        'PRODUCTO_ID_VACIO',
      );
    }
    if (!props.nombre || props.nombre.trim().length < 2) {
      throw new DomainError(
        ...
        'PRODUCTO_NOMBRE_INVALIDO',
      );
    }
    if (!props.categoriaId || props.categoriaId.trim().length === 0) {
      throw new DomainError(
        'El CategoriaId no puede estar vacio',
        'PRODUCTO_CATEGORIA_INVALIDA',
      );
    }
    return new Producto(
      props.id,
      props.nombre.trim(),
      props.categoriaId.trim(),
      true,
    );
  }
  ...
  desactivar(): void {
    if (!this._activo) {
      throw new DomainError(
        'El producto ya esta desactivado',
        'PRODUCTO_YA_DESACTIVADO',
      );
    }
    this._activo = false;
  }
