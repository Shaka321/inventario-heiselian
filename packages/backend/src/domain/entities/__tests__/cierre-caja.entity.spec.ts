...
const props = { id: 'cc-1', usuarioId: 'user-1', montoEsperado: 1000, montoReal: 950 };
...
describe('CierreCaja', () => {
  ...
  it('rechaza montoEsperado negativo', () => {
    expect(() => CierreCaja.crear({ ...props, montoEsperado: -1 })).toThrow(DomainError);
  });
  it('rechaza montoReal negativo', () => {
    expect(() => CierreCaja.crear({ ...props, montoReal: -1 })).toThrow(DomainError);
  });
  it('rechaza usuarioId vacio', () => {
    expect(() => CierreCaja.crear({ ...props, usuarioId: '' })).toThrow(DomainError);
  });
});
