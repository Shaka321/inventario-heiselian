...
export class Compra {
  ...
  static crear(props: {
    ...
    if (!props.id || props.id.trim().length === 0) {
      throw new DomainError(
        'El ID de la compra no puede estar vacio',
        'COMPRA_ID_VACIO',
      );
    }
    if (!props.varianteId || props.varianteId.trim().length === 0) {
      throw new DomainError(
        ...
        'COMPRA_VARIANTE_VACIO',
      );
    }
    if (
      !Number.isInteger(props.cantidadUnidades) ||
      props.cantidadUnidades <= 0
    ) {
      throw new DomainError(
        'La cantidad de unidades debe ser un entero mayor que 0',
        'COMPRA_CANTIDAD_INVALIDA',
      );
    }
    if (!props.proveedorId || props.proveedorId.trim().length === 0) {
      throw new DomainError(
        'El ProveedorId no puede estar vacio',
        'COMPRA_PROVEEDOR_VACIO',
      );
    }
  ...
  get totalCompra(): number {
    return (
      Math.round(this._cantidadUnidades * this._costoUnitario.valor * 100) / 100
    );
  }
