...
describe('CreateCategoriaUseCase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new CreateCategoriaUseCase(mockRepo as any);
  });
  ...
  it('debe lanzar ConflictException si el nombre ya existe', async () => {
    mockRepo.findByNombre.mockResolvedValue({ id: 'uuid-1', nombre: 'Bebidas', activo: true, creadoEn: new Date() });

    await expect(useCase.execute({ nombre: 'Bebidas' })).rejects.toThrow(ConflictException);
  });
});
...
describe('UpdateCategoriaUseCase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new UpdateCategoriaUseCase(mockRepo as any);
  });

  it('debe actualizar una categoria existente', async () => {
    mockRepo.findById.mockResolvedValue({ id: 'uuid-1', nombre: 'Bebidas', activo: true, creadoEn: new Date() });
    mockRepo.findByNombre.mockResolvedValue(null);
    mockRepo.update.mockResolvedValue(undefined);

    await useCase.execute('uuid-1', { nombre: 'Bebidas Frias' });
    expect(mockRepo.update).toHaveBeenCalledWith('uuid-1', { nombre: 'Bebidas Frias' });
  });

  it('debe lanzar NotFoundException si la categoria no existe', async () => {
    mockRepo.findById.mockResolvedValue(null);
    await expect(useCase.execute('uuid-x', { nombre: 'Nueva' })).rejects.toThrow(NotFoundException);
  });

  it('debe lanzar ConflictException si el nombre nuevo ya esta en uso', async () => {
    mockRepo.findById.mockResolvedValue({ id: 'uuid-1', nombre: 'Bebidas', activo: true, creadoEn: new Date() });
    mockRepo.findByNombre.mockResolvedValue({ id: 'uuid-2', nombre: 'Lacteos', activo: true, creadoEn: new Date() });

    await expect(useCase.execute('uuid-1', { nombre: 'Lacteos' })).rejects.toThrow(ConflictException);
  });
});
